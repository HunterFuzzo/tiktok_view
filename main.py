...
"""
    print("")
    print(Colorate.Horizontal(Colors.blue_to_green, Center.XCenter(azuka)))

    item_id = str(input("["+Fore.GREEN+ "?"+ Style.RESET_ALL+"] Enter video link >>> "))
    ...
    if ("vm.tiktok.com" in item_id or "vt.tiktok.com" in item_id):
        ...
        amount = int(input("["+Fore.GREEN+ "?"+ Style.RESET_ALL+"] How many view ? >>> "))
        print("")
        print("[i] Sending view ...")

    if (amount == "0"):
        for _ in iter(int, 1):
            while True:
                if (active_count() <= 5000):
                    print("["+Fore.GREEN+ "+"+ Style.RESET_ALL+"] View sent | ", x)
                    x += 1
